        ...
        <div className="bg-surface text-on-surface min-h-screen flex flex-col items-center">
            ...
            <main className="flex-1 w-full max-w-[390px] px-lg pt-24 pb-32">
                ...
                <section className="mb-xl">
                    <label className="font-label-md text-label-md text-on-surface-variant mb-md block tracking-wider">TITLE</label>
                ...
                <section className="flex flex-col items-center justify-center mb-xl">
                    <label className="font-label-md text-label-md text-on-surface-variant mb-md self-start tracking-wider">TIME</label>
                    <div className="relative w-full max-w-[320px] h-48 flex justify-center items-center bg-white border border-outline-variant rounded-xl overflow-hidden">
                        ...
                        <div className="flex items-center space-x-4 z-10 h-full w-full justify-center">
                            {/* Hours */}
                            <div className="flex flex-col items-center h-full overflow-y-auto no-scrollbar py-20 snap-y snap-mandatory">
                                {Array.from({ length: 12 }, (_, i) => String(i + 1).padStart(2, '0')).map(h => (
                                    <div
                                        key={h}
                                        onClick={() => setHour(h)}
                                        className={`h-12 flex items-center justify-center font-headline-lg text-headline-lg snap-center font-bold cursor-pointer transition-colors ${hour === h ? 'text-primary' : 'text-on-surface-variant'}`}
                                    >{h}</div>
                                ))}
                            </div>
                            ...
                            <div className="flex flex-col items-center h-full overflow-y-auto no-scrollbar py-20 snap-y snap-mandatory">
                                {['00', '05', '10', '15', '20', '25', '30', '35', '40', '45', '50', '55'].map(m => (
                                    <div
                                        key={m}
                                        onClick={() => setMinute(m)}
                                        className={`h-12 flex items-center justify-center font-headline-lg text-headline-lg snap-center font-bold cursor-pointer transition-colors ${minute === m ? 'text-primary' : 'text-on-surface-variant'}`}
                                    >{m}</div>
                                ))}
                            </div>
                            {/* AM/PM */}
                            <div className="flex flex-col items-center h-full overflow-y-auto no-scrollbar py-20 snap-y snap-mandatory ml-2">
                                {['AM', 'PM'].map(p => (
                                    <div
                                        key={p}
                                        onClick={() => setAmpm(p)}
                                        className={`h-12 flex items-center justify-center font-headline-md text-headline-md snap-center font-bold cursor-pointer transition-colors ${ampm === p ? 'text-primary' : 'text-on-surface-variant'}`}
                                    >{p}</div>
                                ))}
                            </div>
                        </div>
            ...
            <footer className="fixed bottom-0 w-full max-w-[390px] bg-surface/80 backdrop-blur-md z-50">
                <div className="p-lg">
                    <button
                        ...
                        className="w-full bg-primary text-on-primary py-md rounded-xl font-headline-md text-headline-md font-bold shadow-lg hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-50"
                    >
                        {saving ? (
